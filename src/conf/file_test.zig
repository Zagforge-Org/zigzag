...
test "loadFromPath returns default for empty file" {
    ...
    const f = try std.Io.Dir.cwd().createFile(tmp_path, .{});
    defer f.close();

    defer {
        std.Io.Dir.cwd().deleteFile(tmp_path) catch |err| {
...
test "loadFromPath parses valid JSON with all fields" {
    ...
    defer f.close();
    defer std.Io.Dir.cwd().deleteFile(tmp_path) catch {};
...
test "writeDefaultConfig writes FileConf.default() to file" {
    ...
    defer std.Io.Dir.cwd().deleteFile(tmp_path) catch |err| {
        std.log.warn("failed to delete file: {}", .{err});
    };
    ...
    // Read all contents
    const file_contents = try std.Io.Dir.cwd().readFileAlloc(std.testing.allocator, tmp_path, 4096);
