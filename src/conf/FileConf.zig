//! FileConf is the structure of zig.conf.json. All fields are optional and missing
//! fields fall back to Config defaults.

const std = @import("std");

pub const DEFAULT_CONF_FILE = "zig.conf.json";
...
pub fn writeDefaultConfig(io: std.Io, full_path: []const u8) !void {
    ...
    try w.interface.writeAll(default());
    try w.interface.flush();
}

/// Loads and parses zig.conf.json from the current directory.
/// Returns null if the file does not exist or is empty.
/// If the file exists but is empty, uses the default configuration.
pub fn load(io: std.Io, allocator: std.mem.Allocator) !?std.json.Parsed(Self) {
    return try loadFromPath(io, allocator, DEFAULT_CONF_FILE);
}
...
/// Loads a FileConf from a JSON file at the given path.
/// Returns null if the file does not exist or is empty.
pub fn loadFromPathEmpty(io: std.Io, allocator: std.mem.Allocator, path: []const u8) !?std.json.Parsed(Self) {
    ...
    return try std.json.parseFromSlice(Self, allocator, slice, .{
...
pub fn loadFromPath(io: std.Io, allocator: std.mem.Allocator, path: []const u8) !?std.json.Parsed(Self) {
    const content = std.Io.Dir.cwd().readFileAlloc(io, path, allocator, .limited(END_ALLOC_SIZE)) catch |err| switch (err) {
        error.FileNotFound => return null,
        else => return err,
    };
