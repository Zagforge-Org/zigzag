...
pub fn init(io: std.Io, allocator: std.mem.Allocator) !Self {
    const fd = try inotify.init1(linux.IN.CLOEXEC);
...
fn addWatchRecursive(self: *Self, path: []const u8) !void {
    if (self.shouldSkipPath(path)) return;

    const path_z = try self.allocator.dupeZ(u8, path);
    defer self.allocator.free(path_z);

    const wd = inotify.addWatchZ(self.ifd, path_z, WATCH_MASK) catch |err| switch (err) {
    ...
    if (gop.found_existing) {
        ...
        // Subtree was already fully walked.
        return;
    }
...
fn removeWatchByPath(self: *Self, dir_path: []const u8) void {
