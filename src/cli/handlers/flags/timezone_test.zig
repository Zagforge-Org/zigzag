...
test "handleTimezone handles timezone option" {
    ...
    try handleTimezone(std.testing.io, &cfg, allocator, "+5");
    try testing.expectEqual(@as(i64, 18000), cfg.timezone_offset);

    try handleTimezone(std.testing.io, &cfg, allocator, "-3:30");
...
test "handleTimezone handles invalid timezone option" {
    ...
    const result_invalid_mins = handleTimezone(std.testing.io, &cfg, allocator, "+5:60");
    try testing.expectError(error.InvalidTimezoneMinutes, result_invalid_mins);

    const result_invalid_format = handleTimezone(std.testing.io, &cfg, allocator, "-3:30:00");
