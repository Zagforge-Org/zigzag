...
test "handleChunkSize: bare integer" {
    ...
    defer cfg.deinit();
    try handleChunkSize(std.testing.io, &cfg, std.testing.allocator, "500000");
...
test "handleChunkSize: k suffix" {
    var cfg = Config.default(std.testing.allocator);
    defer cfg.deinit();
    try handleChunkSize(std.testing.io, &cfg, std.testing.allocator, "500k");
    try std.testing.expectEqual(@as(usize, 500_000), cfg.llm_chunk_size);
}
...
test "handleChunkSize: m suffix" {
    var cfg = Config.default(std.testing.allocator);
    defer cfg.deinit();
    try handleChunkSize(std.testing.io, &cfg, std.testing.allocator, "2m");
...
test "handleChunkSize: zero is error" {
    var cfg = Config.default(std.testing.allocator);
    defer cfg.deinit();
    try std.testing.expectError(error.InvalidChunkSize, handleChunkSize(std.testing.io, &cfg, std.testing.allocator, "0"));
}

test "handleChunkSize: invalid suffix is error" {
    var cfg = Config.default(std.testing.allocator);
    defer cfg.deinit();
    try std.testing.expectError(error.InvalidChunkSize, handleChunkSize(std.testing.io, &cfg, std.testing.allocator, "500x"));
}

test "handleChunkSize: null value is no-op" {
    var cfg = Config.default(std.testing.allocator);
    defer cfg.deinit();
    try handleChunkSize(std.testing.io, &cfg, std.testing.allocator, null);
