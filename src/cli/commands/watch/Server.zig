...
//!
//! Architecture: single event-loop thread using periodic ticks for I/O no thread-per-client.
//! Connected SSE clients are held in a flat registry; broadcast() stores the new payload
//! under a mutex and returns immediately.
...
pub fn init(io: std.Io, port: u16, root_dir: []const u8, default_page: []const u8, allocator: std.mem.Allocator) !*Server {
...
pub fn broadcast(self: *Server, payload: []const u8) void {
...
pub fn broadcastCombined(self: *Server, payload: []const u8) void {
    const copy = self.allocator.dupe(u8, payload) catch return;
...
pub fn broadcastReload(self: *Server) void {
    self.mu.lockUncancelable(self.io);
...
pub fn openBrowser(self: *const Server) void {
...
fn upgradeSse(
    ...
    streamWriteAll(self.io, stream, headers) catch {
        socket.gracefulClose(self.io, stream);
        return;
    };
...
/// Write all bytes to a stream through a buffered writer, then flush.
...
fn writePartsToAll(io: std.Io, clients: *std.ArrayList(std.Io.net.Stream), parts: []const []const u8) void {
    ...
    while (i < clients.items.len) {
        ...
        if (!alive) {
            socket.gracefulClose(io, clients.items[i]);
...
fn serveStatic(io: std.Io, allocator: std.mem.Allocator, root_dir: []const u8, default_page: []const u8, req_path_raw: []const u8, stream: std.Io.net.Stream) void {
    defer socket.gracefulClose(io, stream);
...
fn openBrowserThread(io: std.Io, allocator: std.mem.Allocator, url: []u8) void {
    ...
    browser.openUrl(io, allocator, url);
}
